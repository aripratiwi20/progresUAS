...
class CustemerScreen extends React.Component {
  ...
  _onRefresh() {
    ...
    fetch('http://mhs.rey1024.com/appmobile/D1615051093/getData.php')
  ...
  return fetch('http://mhs.rey1024.com/appmobile/D1615051093/getData.php')
    .then((response) => response.json())
    .then((responseJson) => {
      let ds = new ListView.DataSource({rowHasChanged: (r1, r2) => r1 !== r2});
      this.setState({
    ...
    return (
      <TouchableOpacity activeOpacity={0.7} style={styles.row}>
         ...
         <View style={styles.tulisan}>
          ...
          <Text style={styles.Title}>bahan : {record.bahan}</Text>
          <Text style={styles.Details}>langkah : {record.langkah}</Text>
          </View>
         </View>
		...
		return(
			<ImageBackground
              ...
              <View style={styles.box}>
        			<Text style={styles.title}>MY ONE RECEIPT</Text>
       				<Text style={styles.subTitle}>Datail resep</Text>
                <ListView
                  refreshControl={
                  <RefreshControl
